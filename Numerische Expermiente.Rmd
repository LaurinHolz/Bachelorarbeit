...
```{r}
it <- 1000 #Anzahl der Durchläufe
max_percentage <-25 #Anzahl des maximalen Prozentsatzes an fehlenden Daten
...
for (k in 1:it) {
  ...
  for (j in 1:max_percentage) {
    df <- read_excel("Felddaten.xlsx")
    #df <- df[,-c(4,5,6)] #Für einzelene Spalten, lösche Variablen hier
    entries <- nrow(df)*ncol(df)
