...
regression_imp <- function(x){
  ...
  for (i in 2:p) {
    position <- which(is.na(x[,i]))
    age <- which(is.na(x[,i]))+x[1,1]-1
    fit <- lm(x[,i]~x[,1])
    tmp <- summary(fit)
    coff1 <- tmp$coefficients[[1]]
    coff2 <- tmp$coefficients[[2]]
    x[position,i] <- coff2*age+coff1
  }
  x <- as.data.frame(x)
  return(x)
}
...
em_spline <- function(data){
  
  col <- colnames(data)
  for (i in 2:ncol(data)) {
    f <- reformulate(c(col[1],col[i]))
    data_mis <- cbind(data[,1],data[,i])
    
    if(anyNA(data_mis)){
      tmp <- mtsdi::mnimput(f,data_mis)$filled
      data[,i] <- tmp[,2]
    }else{
      next
    }
...
em_algorithm <- function(data,imputations=1,maxIt=1000,criterion=0.0001,rseed,start.mean,start.var){
